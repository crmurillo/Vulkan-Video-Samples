...
int32_t VkVideoDecoder::StartVideoSequence(VkParserDetectedVideoFormat* pVideoFormat)
{
    ...
    if (m_enableDecodeComputeFilter) {
        ...
        // Only recreate filter if parameters changed that affect shader/pipeline
        bool needNewFilter = true;
        if (m_yuvFilter) {
            VulkanFilterYuvCompute* yuvFilter = static_cast<VulkanFilterYuvCompute*>(m_yuvFilter.Get());
            if (yuvFilter->IsCompatible(
                    m_filterType,
                    numDecodeSurfaces + 1,
                    inputFormat,
                    outputFormat,
                ...
                m_yuvFilter = nullptr;  // Destroy incompatible filter
            }
        }
